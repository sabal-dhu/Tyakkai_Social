...
export default function ContentCalendarPage() {
  ...
  const dummyPosts = [
    ...
    },
    {
      id: 9,
      content:
        "Check out our latest blog post on content marketing strategies.",
      platforms: ["facebook", "twitter"],
      scheduled_datetime: new Date(
        currentDate.getFullYear(),
        currentDate.getMonth(),
    ...
    },
    {
      id: 10,
      content:
        "Join us for a live Q&A session with our CEO tomorrow at 3 PM EST.",
      platforms: ["instagram", "facebook"],
      scheduled_datetime: new Date(
        currentDate.getFullYear(),
        currentDate.getMonth(),
    ...
    try {
      ...
      try {
        ...
        const platformsResponse = await axios.get(
          "http://localhost:8000/api/platforms/available",
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const handleDragStart = (e, post) => {
    ...
    if (e.dataTransfer) {
      ...
      dragImage.innerHTML = `
        <div style="padding: 8px; background-color: ${getCampaignColor(
          post.campaign
        )}; 
                    color: white; border-radius: 4px; width: 150px; 
                    border-left: 3px solid ${getPlatformColor(
                      post.platforms[0]
                    )};">
  ...
  const handleDrop = async (e, date, hour) => {
    ...
    try {
      ...
      try {
        await axios.put(
          `http://localhost:8000/api/posts/${draggedPost.id}`,
          updatedPost,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
      ...
      // Update local state
      setPosts(
        posts.map((post) => (post.id === draggedPost.id ? updatedPost : post))
      );
    } catch (err) {
                  ...
                  <div className="calendar-grid-body">
                    {generateMonthCalendar().map((day, index) => (
                      <div
                        ...
                        onDragOver={
                          day.date ? (e) => handleDragOver(e, day.date) : null
                        }
                        onDragLeave={day.date ? handleDragLeave : null}
                        onDrop={
                          day.date ? (e) => handleDrop(e, day.date) : null
                        }
                        onClick={
                          day.date ? () => handleCreatePost(day.date) : null
                        }
                      >
                        {day.day && (
                          <>
                            ...
                            <div className="calendar-cell-content">
                              {getPostsForDate(day.date).map((post) => (
                                <div
                                  ...
                                  style={{
                                    backgroundColor: getCampaignColor(
                                      post.campaign
                                    ),
                                    borderLeft: `3px solid ${getPlatformColor(
                                      post.platforms[0]
                                    )}`,
                                  }}
                                  onClick={(e) => {
