...
export default function HashtagSuggestionsPage() {
    ...
    const fetchSavedHashtags = async () => {
      ...
      try {
        ...
        try {
          ...
          const response = await axios.get("http://localhost:8000/api/hashtags/saved", {
    ...
    try {
      ...
      try {
        ...
        const response = await axios.post(
          "http://localhost:8000/api/hashtags/generate",
          {
            content,
            platform,
            industry,
  ...
  const saveHashtagGroup = async () => {
    ...
    try {
      ...
      try {
        const token = localStorage.getItem("access_token")
        await axios.post(
          "http://localhost:8000/api/hashtags/save",
          {
            name: groupName,
  ...
  const deleteHashtagGroup = async (id) => {
    ...
    try {
      ...
      try {
        const token = localStorage.getItem("access_token")
        await axios.delete(`http://localhost:8000/api/hashtags/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
      } catch (error) {
        console.log("API not available, updating local state only")
            ...
            <div className="card-body">
              ...
              <div className="row mb-3">
                ...
                <div className="col-md-4">
                  ...
                  <select className="form-select" id="count" value={count} onChange={(e) => setCount(e.target.value)}>
              ...
              <div className="d-grid">
                <button className="btn btn-primary" onClick={generateHashtags} disabled={!content.trim() || generating}>
                  {generating ? (
                    <>
            ...
            <div className="card mb-4">
              <div className="card-header d-flex justify-content-between align-items-center">
                ...
                <div>
                  ...
                  <button className="btn btn-sm btn-outline-success" onClick={saveHashtagGroup}>
                    Save Group
                  </button>
                </div>
              </div>
            ...
            <div className="card-body">
              {loading ? (
                ...
                <div className="alert alert-info">
                  No saved hashtag groups. Generate and save hashtags to create your first group.
                </div>
              ) : (
                <div className="accordion" id="hashtagGroups">
                  {savedHashtags.map((group) => (
                    <div className="accordion-item" key={group.id}>
                      ...
                      <div id={`group-${group.id}`} className="accordion-collapse collapse">
